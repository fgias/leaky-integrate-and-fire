...
def run(N, R, sigma, time_max, seed_num, lmd):
	...
	while t < time_max:
		...
		if round(t,2) == t_snapshot or round(t,2) == t_snapshot/2:
			"""
			Saving data.
...
for lmd in np.arange(0.90, 0.89, -0.01):
	run(N, round(0.4*N), 1.7, 1000, seed_num, round(lmd, 2))

for lmd in np.arange(0.8, -0.1, -0.1):
	run(N, round(0.4*N), 1.7, 1000, seed_num, round(lmd, 1))
