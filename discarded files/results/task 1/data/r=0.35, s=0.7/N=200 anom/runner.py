...
def run(N, R, sigma, time_max):
	...
	plt.xticks([1, N/2, N])
	plt.xlabel("index i")
	plt.savefig('w_N=%s.png' % N, dpi=800)
	plt.show()
	...
	plt.xticks([1, N/2, N])
	plt.xlabel("index i")
	plt.savefig('u_N=%s.png' % N, dpi=800)
	plt.show()

	delta_w = max(w) - min(w)

	with open('deltaw_data.csv', 'a', newline='') as file:
		writer = csv.writer(file)
		writer.writerow([delta_w])

	u_data = asarray(u)
	w_data = asarray(w)
	savetxt('u_data_N=%s.csv' % N, u_data, delimiter=',')
	savetxt('w_data_N=%s.csv' % N, w_data, delimiter=',')
	with open('calc_N=%s.txt' % N, 'w') as f:
		f.write('Results:')
		f.write('\n')
