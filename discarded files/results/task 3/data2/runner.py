...
def run(N, R, sigma, time_max):
	...
	plt.xticks([1, N/2, N])
	plt.xlabel("index i")
	plt.savefig('data2/u_N=%s.png' % N, dpi=800)
	plt.show()

	delta_w = max(w) - min(w)

	with open('data2/deltaw_data.csv', 'a', newline='') as file:
		writer = csv.writer(file)
		writer.writerow([delta_w])

	u_data = asarray(u)
	w_data = asarray(w)
	savetxt('data2/u_data_N=%s.csv' % N, u_data, delimiter=',')
	savetxt('data2/w_data_N=%s.csv' % N, w_data, delimiter=',')
	with open('data2/calc_N=%s.txt' % N, 'w') as f:
		f.write('Results:')
		f.write('\n')
